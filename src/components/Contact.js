...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.code == 200) {
      setStatus({ succes: true, message: 'Le message est envoyé.'});
    } else {
      setStatus({ succes: false, message: 'Il y a eu un problème, veuillez réessayer.'});
    }
  };
              ...
              {({ isVisible }) =>
                ...
                <form onSubmit={handleSubmit}>
                  <Row>
                    ...
                    <Col size={12} sm={6} className="px-1">
                      <input type="text" value={formDetails.lasttName} placeholder="Nom de famille" onChange={(e) => onFormUpdate('lastName', e.target.value)}/>
                    </Col>
                    <Col size={12} sm={6} className="px-1">
